readandprintkbps <- function(bits) {
  patt <- paste("alltest",bits,"bits_[1-5].csv", sep = "")
...
readandprintpps <- function(bits) {
  ...
  #print them all to file!
  filename <- paste("alltest",bits,"bit_pps_vs_rules.pdf", sep="")
  pdf(file = filename)
  for (p in x) {
    title = paste(bits, "bits,", p$number.of.rules[[1]], "Rules")
    xlabel = "Number of Tables"
    ylabel = "PPS"
    plot(p$number.of.tables, p$pps, main = title, xlab = xlabel, ylab = ylabel)
  }
  dev.off()
}

readandprintrealpps <- function(bits) {
  patt <- paste("alltest",bits,"bits_[1-5].csv", sep = "")
  files <- lapply(c(list.files(pattern = patt)), read.csv, head=T)
  total <- data.frame()
  for (file in files[]) {
  ...
  #print them all to file!
  filename <- paste("alltest",bits,"bit_realpps_vs_rules.pdf", sep="")
  pdf(file = filename)
  for (p in x) {
    title = paste(bits, "bits,", p$number.of.rules[[1]], "Rules")
    xlabel = "Number of Tables"
    ylabel = "PPS"
    plot(p$number.of.tables, 500000 / p$real.process.time, main = title,
